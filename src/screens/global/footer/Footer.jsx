      ...
      <Container>
        ...
        <div className='footer-bottom flex items-center justify-between'>
          <Paragraph className='copyright-text'>
            &copy; {currentYear.getFullYear()} Streamvibe. All Rights Reserved.{}
          </Paragraph>
...
const FooterItem = ({ data }) => {
  return (
    <FooterItemWrapper>
      <h4 className='item-title text-xl capitailze font-semibold'>{data.link_title}</h4>
        ...
        <ul className='item-icons-links flex items-center items-wrap'>
          {data?.links?.map((link, index) => (
            <li key={index}>
              ...
              >
                <img src={link.link_icon} alt='link icon' />
              </Link>
            </li>
          ))}
        </ul>
